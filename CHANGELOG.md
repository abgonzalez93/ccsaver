...
- the file had reached the 350 lines of the gate; the three tests about delegate, fail and note lines are about the log and move to test/state/log.test.ts, where the fixtures already were

## 0.33.12 - 2026-09-24

refactor: the usage table and the answer to a mistake on the command line move to src/cli/usage.reporter.ts

- ccsaver.cli.ts sat at 347 lines; the table, the row a mistake prints under its Error: line and the helpers that check an argument count are what a command prints when it stops, a reporter of their own
- the CLI stays at the top of src, where bin/ccsaver finds it
...
- the file produces the denied: line as well as the spent: one, and both come from folding the month's log
...
- the hook let a numeric pages through as a ranged read but wrote nothing about it in the gate line, so saved would have weighed a small PDF as the whole file

## 0.33.5 - 2026-09-24

fix: the reason doctor quotes for a rejected request stays on one line

- an error.message with a line break split the FAIL probe: line in two, and the slash command reads the report line by line; runs of whitespace are now one space
...
- src/delegation/ sat at five files with worker.client.ts at 348 lines, so the next change to the transport would have had to split a file and regroup a folder in one commit; boundary.guard.ts and answer.validator.ts move to src/boundary/, and invokeClaude with its bare, bounded child moves to fallback.client.ts
- the hook imports the same four modules by another path: 24.7 / 24.8 / 25.0 ms before against 25.0 / 25.2 / 25.0 after on a 100-line file let through, three rounds of 30 paired runs, log on

## 0.31.0 - 2026-09-24

feat: handoff write says when it replaced the handoff of the session

- there is one handoff per session, and a second write in the same session overwrote the first while printing kept; now it prints replaced, and the written event says so
...
- doctor held the whole month in an array, 263 MB of resident memory on a 56 MB month, where saved already folds; the reader that built that array, readEvents, had no other caller and is gone
- the map of CONTRIBUTING 2.1 gains the row, and its rows for doctor.service.ts and ccsaver.cli.ts name handoff.store, which both import, and drop worker.client from the CLI, which never did

## 0.29.9 - 2026-09-24

fix: the launcher's key set line carries the session, and usage says setup writes the launcher

- docs/events.md promises the session id on every line and the one line the shell writes said null even inside a session
- the usage row of setup left out the launcher it has written since 0.23.0, while the README table named it

## 0.29.8 - 2026-09-24

fix: the sh gate reads a compact handoff.json as off, not only the one ccsaver writes

- the shortcut matched the two-space spelling of JSON.stringify alone, so a hand-written {"on":false} started Node on every turn, 24 ms instead of 2, and the page said otherwise

## 0.29.7 - 2026-09-24

fix: doctor's denied line counts only the whole-file reads the hook could judge

- a read of a file outside the root, of a binary, of a missing file or from a malformed call can never be denied, and each one sat in the denominator, so a session of subagents reading outside the root diluted the share the line reports
...
- a denial of such a file was already left out of without, because nothing could have delegated it, but its ranged reads were still charged to ccsaver: 1,000 tokens for one 100-line range of a 400 KB file, and in months before 0.24.13 enough to move the sign of the month

## 0.29.3 - 2026-09-24

docs: CONTRIBUTING 5.3 and 5.5 point at docs/versions.md, so the next rule has room under the 32 KB gate

- the file was 17 bytes under its own limit; the case-by-case behaviour of the version hook and the CI notes it repeated already live in docs/versions.md

## 0.29.2 - 2026-09-24

chore: the repository ignores .claude/settings.local.json itself

- it was ignored only by the global git configuration of one machine, so another clone would have shown it as untracked and a git add . would have committed it
...
- 1.4 says why `readEvents` and `pagedAfter` write into the rows and the table they carry: a copy per row is quadratic, and the note has the numbers
- two sentences of 1.6 and 1.8 lost what they repeated, to keep the file under its own gate
...
- development.md describes the paired harness behind the recent hook numbers: a throwaway state folder with the log on, a 300 KB transcript, a worktree of the tree before, 30 runs in turn per arm and three rounds, and the floor two identical trees sit apart

## 0.25.6 - 2026-09-24

refactor: what followed a denial is summed in the service, and the hook and the survey share one scan ceiling

- `followedIn` and `Followed` move from `saved.reporter.ts` to `saved.service.ts`, beside the fold that fills the table they sum
- `SCAN_CEILING` in the hook and `CEILING_BYTES` in the survey were the same megabyte; it is one export of `config.store.ts` now, and the hook measured the same before and after
- events.md lists the six things `saved` cannot see in one place, the re-read context included, instead of five and a paragraph apart
- configuration.md opens the limits in three sentences instead of one
