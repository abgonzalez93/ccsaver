...
Claude Code's documentation says a skill's grant applies for the turn whether you type `/ccsaver:bulk-reader` or Claude invokes it; on 2.1.274 the usual permission flow was seen to decide when Claude invoked it after the hook's message. If it asks, answer "don't ask again" once, or add both rules to `permissions.allow` in `~/.claude/settings.json`.
