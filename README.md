...
Measured on one TypeScript monorepo with Claude Code 2.1, small samples of 1–4 sessions per cell. Every number and its sample size is in [docs/measurements.md](docs/measurements.md), and what ccsaver itself costs in [docs/development.md](docs/development.md); [measure your own](docs/development.md#measure-it-yourself) before believing any of it.
...
| Hook overhead per `Read` | [**1–3 ms** unplugged, **≈ 4 ms** plugged with the log off and no adapter, **≈ 24 ms** with either](docs/development.md#hook-overhead-per-read) |
...
- The context warning reads the tail of the session transcript, up to its last 256 KB, on your machine, for one token count and the model's name; nothing else is read from it, nothing from it is sent anywhere, and those two are recorded only while the [event log](docs/events.md) is on.
