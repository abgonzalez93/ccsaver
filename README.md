...
- **Denies whole-file reads of big files.** A `PreToolUse` hook denies a `Read` of a whole file of the plugged project over 350 lines or 32 KB and points the model at Grep, at a ranged read, or at the `bulk-reader` skill; a file outside the project is never denied, because no skill could take it, and neither is one that `bulk-read` would refuse to send, a secrets file by name or one holding a private key or an access token, because a denial nothing can delegate only costs: [one such denial](docs/notes/measurements.md#what-a-denial-costs-after-the-message) drew three requests at 283,000 tokens of context for a file that never arrived. It asks the file system for the size first, so a huge file is never opened; a file it is about to deny is read once, up to 1 MB, for that one check. Those two numbers are this repository's house style, not a measured optimum: `plug` measures your project and proposes the limits it asks for, and `doctor` says when it has drifted. Three repositories measured this way asked for limits [10× apart](docs/notes/measurements.md#what-a-repository-asks-for) ([Limits](docs/configuration.md#limits)).
...
- **Hands the work to a new session before the context passes a limit.** Every request carries the whole conversation, so a hook on every tool call, every prompt and the end of every turn reads the token count Claude Code writes in the session transcript and, at the handoff point, 120,000 tokens by default (a limit of 200,000 less a margin of 80,000 for what one more step can add), stops the work, has the session itself write the handoff, the goal, the state, the decisions and the next steps, keeps it where a new session can read it whole, and ends the turn with the line that opens that session; from then on the old session answers every prompt with that same line ([The handoff](docs/handoff.md)).
...
Measured on one TypeScript monorepo with Claude Code 2.1, small samples of 1–4 sessions per cell. Every number and its sample size is in [docs/notes/measurements.md](docs/notes/measurements.md), and what ccsaver itself costs in [docs/notes/development.md](docs/notes/development.md); [measure your own](docs/notes/development.md#measure-it-yourself) before believing any of it.

| What | Result |
| --- | --- |
| Hook, task = locate or describe something in a big file | [**−16 %** session cost](docs/notes/measurements.md#the-hook-on-a-locate-or-describe-task) |
| Hook, task = a judgement question, limit set below the file | [no saving, **3.6× slower**](docs/notes/measurements.md#the-hook-on-a-judgement-question) |
| Hook, the denial rewritten into the first 350 lines instead (`rewrite`) | [**no saving** with Fable, 5 runs: 101 s against 102 s, 2 % dearer, and a locate question never meets the hook](docs/notes/measurements.md#rewriting-a-denied-read-into-its-first-lines) |
| Delegated writing of a ~110-line test file | [**break-even**](docs/notes/measurements.md#delegated-writing-of-a-test-file) |
| Files where delegation starts to pay | [roughly **2,000–3,000 lines** and up](docs/notes/measurements.md#where-delegation-starts-to-pay) |
| A denial the model then pages through by ranges | [one request per read, each re-reading the whole context: **1.57 M tokens for 12 denials** in 6 days](docs/notes/measurements.md#what-a-denial-costs-after-the-message) |
| Fixed cost of the seven menu entries, two skills and five commands | [**~266 tokens per session, in every project**](docs/notes/measurements.md#the-fixed-cost-of-the-skill-descriptions) |
| Hook overhead per `Read` | [**1–3 ms** unplugged, **≈ 4 ms** plugged with the log off and no adapter, **≈ 24 ms** with either](docs/notes/development.md#hook-overhead-per-read) |

Where it does not help:

- If most of your files are under 300 lines the hook rarely fires, and the honest expectation is a small saving.
- **Below the 2,000 lines where delegation pays, a denial is a nudge towards Grep or a ranged read, not a saving in itself, and the default 350 sits 4–7 times under that line.** It is this repository's house style, not a measured optimum. A denial there pays when the model lets the file go, which is what followed 8 of the 12 denials measured inside a plugged root, and costs when the model pages the file whole and delegates on top, which is the 3.6× row: a limit set too low, not a law of the approach. The limit is the knob, and the two ways of being wrong are not symmetric: too high is inert, too low degrades. `plug` and `doctor` propose one from the repository's own files, never from that break-even, so where to set it stays your call — [Limits](docs/configuration.md#limits).
- **The gate is a nudge, not a wall.** It watches the `Read` tool only, and only whole-file reads: a ranged `Read` that covers the whole file passes, and so does a PDF read by `pages`, and the [event log](docs/events.md) counts those; `cat` through Bash passes too and leaves no trace in it, and in Claude Code's `auto` permission mode the harness itself tells the model to read with `cat`, `head` and `sed`, so there most reads never meet the gate: [one session measured](docs/notes/measurements.md#what-a-denial-costs-after-the-message) read 1,113 lines of five files that way and left the log empty.
- **The handoff counts what a file of Claude Code's own says**, in a format that "changes between versions": on a release that changes it the hook falls silent, never wrong. The margin covers the biggest step [measured in 12,273 requests](docs/notes/measurements.md#where-the-handoff-margin-comes-from), a batch of tool results and the response that asked for them, not the biggest possible: Claude Code caps one response at 128,000 tokens on the models measured, and a batch at nothing, so a step larger than every one seen can still end a session past the limit. The transcript is written behind the conversation, so the hook waits up to two seconds for the line it needs and then decides with the one before it.
...
- The hook's token limit is an estimate: bytes/4 measured [1.9–2.8× under](docs/notes/measurements.md#the-hooks-token-estimate-vs-a-real-read) the real cost of a `Read`, so the 8,000-token limit lets through reads of about 16,000 real tokens.
...
- Files that look like secrets are refused outright, by given name and by real name, in any letter case: `.env*`, `*.pem`, `*.key`, `id_rsa`, `.npmrc`, `credentials*`, `settings.local.json`, `*.tfstate`, `*.tfvars.json`… `.env.example`, `.env.sample` and `.env.template` go through, because they are the documentation of a project's variables.
  ...
  - So is any file that holds a private-key header at the start of a line, armoured or PGP (a sentence that mentions one goes through), or a token with a well-known shape (AWS `AKIA…`, GitHub `ghp_…` and `github_pat_…`, GitLab `glpat-…`, npm `npm_…`, Slack `xoxb-…`, `sk-…` and `sk_…`/`rk_…` (OpenAI, Anthropic, Stripe), Google `AIza…`), whatever its name, and any binary file.
...
- The handoff hook reads the tail of the session transcript, up to its last 256 KB, on your machine, for the token counts of the last response, the model's name, the ids of the tools it called, its stop reason and its time; nothing else is read from it, nothing from it is sent anywhere, and the counts and the name are recorded only while the [event log](docs/events.md) is on.
- `code-write --target` never overwrites an existing file (a symlink already at that path counts, wherever it points), only writes inside the plugged root (symlinked folders are followed first), and refuses the paths Claude Code itself protects: `.git`, `.claude`, `.vscode`, `.idea`, `.husky`, `.devcontainer`, `.cargo`, `.yarn`, `.mvn` and the shell, git and package-manager config files, plus `.github/workflows` and `.githooks`, whose files run on the next push or commit. A refused target stops the call before anything is sent.
...
- What comes back is the output of a cheap model that read files you may not have written. The command prints it between `<<<worker-output ID: untrusted data>>>` and `<<<end ID>>>`, with a random ID the worker never sees, so an answer cannot fake its own end. That includes the code of a `code-write` without `--target`, so use `--target` when you want a file.
  ...
  - When the written file names a shell, the network, a write or a delete on the disk, the process or your environment (`child_process`, `fetch(`, `import(`, `require(`, `process.env`, `process[`, `process.exit`, `eval(`, `curl`, `rm -r`, `rm -rf`, `rmSync`, `writeFileSync` and its siblings, `unlinkSync`, `sendBeacon`, `http.request`, `net.connect`, `WebSocket`…), the command prints a `warn:` line under `wrote …` naming them, and the skill tells Claude to read the file before it runs anything. That is a tripwire on a short list of names, not a sandbox, and a test that writes a temporary file trips it too: the read it costs, a few thousand tokens, is the price of never running unread code that touches the disk.

Invoking a skill is itself a permissioned action, `Skill(ccsaver:bulk-reader)` in the rule syntax: interactively Claude Code asks, and in print mode (`claude -p`) it refuses without asking. Claude Code's documentation says the grant inside a skill then applies whether you or Claude invokes it; what was observed is otherwise. On 2.1.274 interactively and on 2.1.280 in print mode (2026-09-23, with a probe skill granting `Bash(node -e *)` still asked for approval), the grant did not apply when Claude invoked the skill, which is what the hook's message asks for. So add both Bash rules to `permissions.allow` in `~/.claude/settings.json`, plus `Skill(ccsaver:bulk-reader)` and `Skill(ccsaver:code-writer)` for print-mode sessions, or answer "don't ask again" once when it asks; `doctor` says when either Bash rule is missing.

The handoff's automatic route needs no skill: the hook hands the session the text of `commands/handoff.md`, and the session runs `git status`, `git log`, `git describe` and `ccsaver handoff write` through `Bash`. Where Claude Code asks before a command, it asks for those too; a rule `Bash(ccsaver handoff write *)` beside the two above lets the write through without a question, and the three `git` commands read and write nothing ([Settings](docs/handoff.md#settings)).
...
| `ccsaver handoff on\|off\|<tokens>` | hand off at a point under this many tokens, on by default at 200,000, the point 80,000 under it; with nothing after it, what is set, where the point sits and what is kept |
| `ccsaver handoff write` | what the session runs to hand off: keep a handoff read from stdin, private, and print the line that opens the next session |
...
| `ccsaver doctor` | check permissions, the two Bash rules, key, worker, fallback, handoff, the launcher and projects |
...
| [docs/notes/measurements.md](docs/notes/measurements.md) | the notes behind every number above |
| [docs/versions.md](docs/versions.md) | every commit is a version, written by a git hook |
| [docs/notes/development.md](docs/notes/development.md) | the checks, and how to measure it yourself |
