...
import { AS_ROOT, userLine } from "../test.helpers.ts"
import {
  askedOf,
  assistant,
  batch,
...
test("a batch near the point is cut: the calls that fit pass, the rest are denied, and one held line records the batch", async () => {
  ...
  const path = transcript(userLine(), ...batch(105_000, ids, 0))
  ...
  const far = transcript(userLine(), ...batch(60_000, ids))
...
test("a turn whose last line is an old end of turn is waited on for two seconds, then judged by it", async () => {
  ...
  const path = transcript(userLine(), assistant(126_000, { at: stale }))
  const started = Date.now()
...
test("a handoff kept before the hook asked leaves the session asked, not written", async () => {
  ...
  const path = transcript(userLine(), ...batch(120_000, ["toolu_o"]))
  assert.match(denial(await tool(path, "toolu_o", "session-o")), REQUEST)
  const next = transcript(userLine(), ...batch(121_000, ["toolu_o2"]))
...
test("a subagent's call is left alone, whatever the session's context", async () => {
  const path = transcript(userLine(), ...batch(500_000, ["toolu_sub"]))
...
test("a transcript that is missing, unreadable, of another shape, or cut before its last usage leaves the event in silence, with no crash", async () => {
  ...
  const paths = [
    ...
    transcript(userLine(), userLine()),
    transcript(userLine(), assistant(500_000), userLine("y".repeat(300_000))),
  ]
  ...
  assert.deepEqual(await stop(transcript(userLine(), assistant(500_000)), null), QUIET)
  if (!AS_ROOT) {
    const shut = transcript(userLine(), assistant(500_000))
...
test("handoff.json off keeps the hook quiet, a limit of its own moves the point, and a broken one leaves a crash and silence", async () => {
  ...
  assert.deepEqual(await stop(transcript(userLine(), assistant(300_000)), "session-k"), QUIET)
  writeFileSync(file, JSON.stringify({ limit: 150_000 }))
  assert.match(
    contextOf(await stop(transcript(userLine(), assistant(80_000)), "session-k")),
  ...
  assert.deepEqual(await stop(transcript(userLine(), assistant(900_000)), "session-k"), QUIET)
  ...
  assert.deepEqual(await stop(transcript(userLine(), assistant(50_000)), "session-k"), QUIET)
...
test("markers of other sessions older than seven days are swept when a marker is written, and a kept handoff never is", async () => {
  ...
  await stop(transcript(userLine(), assistant(200_000)), "session-l")
  assert.deepEqual(
...
test("a session id that is no file name is made one before it names a marker", async () => {
  await stop(transcript(userLine(), assistant(200_000)), "../../etc/x y")
