...
test("a private adapter shadows the bundled one, and a malformed one is rejected", () => {
  ...
  writeFileSync(join(HOME, "adapters", "cut-badly.json"), JSON.stringify({ rewrite: "yes" }))
  assert.deepEqual(loadAdapter("cut"), { rewrite: true })
  assert.throws(() => loadAdapter("cut-badly"), /malformed/)
