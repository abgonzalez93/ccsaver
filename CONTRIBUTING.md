...
There is one per flow that needs judgement, **never one per command**. `key set`, `worker set`, `fallback` and `launcher write` live inside `setup.md` because they need the warnings around them; `unplug` and `list` are a line each in `plug.md`; `price` lives inside `saved.md`, because a price with no report is meaningless; `bulk-read` and `code-write` are the skills' own. A new one earns its place by carrying what a summary would drop first: `saved` its band, `handoff` its eight parts and the size a `Read` takes. Each pays for its place in a description every session loads, counted in `docs/measurements.md` with the skill descriptions (~266 tokens).
