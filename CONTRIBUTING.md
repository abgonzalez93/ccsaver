...
Guard: *convention*, measured with `pnpm exec biome lint --only=complexity/noExcessiveCognitiveComplexity src test`. The last two over it, `gate` in `src/read-gate.hook.ts` (18) and `invokeExternal` in `src/delegation/worker.client.ts` (16), came under by splitting what each one decided from what it did: `answered` writes the gate's answer, `usableKey` and `answerOf` check the key and read the response. A new function over 15 is split before it lands; three that resist put the rule in `biome.json` as an error.
...
| `src/state/config.store.ts` | what the user configured: the plugged roots, the adapters and their limits, and the last line of the session's transcript | `log.store`, `measure.helpers`, `state.store` |
| `src/state/handoff.store.ts` | `handoff.json`, the switch and the limit of the context warning, and the `handoff/` folder | `log.store`, `measure.helpers`, `state.store` |
...
| `src/read-gate.hook.ts` | the `Read` gate | `boundary.guard`, `config.store`, `log.store`, `measure.helpers`, `state.store` |
