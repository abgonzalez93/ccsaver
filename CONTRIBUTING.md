...
| `src/doctor/month.service.ts` | the two lines that add the month's log up, `denied:` and `spent:`, folded one row at a time | `config.store`, `finding.reporter`, `log.store`, `state.store` |
| `src/doctor/doctor.service.ts` | every check `doctor` runs and the level each one reports | `config.store`, `environment.service`, `fallback.client`, `finding.reporter`, `handoff.store`, `log.store`, `month.service`, `state.store`, `survey.service`, `terminal.reporter`, `worker.client`, `worker.store` |
