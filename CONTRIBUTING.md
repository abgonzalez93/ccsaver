...
The exception is `.github/workflows/release.yml`, whose single job takes `contents: write`, because publishing a GitHub release is a write to the repository. It is kept apart from `ci.yml` so that the workflow which runs on every push and every pull request keeps its read-only token. It runs on a `v*.*.0` tag and nowhere else, installs nothing, and its one command is `gh release create` with the section `CHANGELOG.md` already carries. Nothing else is given a write token.
Guard: *convention*.
