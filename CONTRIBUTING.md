...
| `src/doctor/spent.service.ts` | the two lines that add the month's log up, `denied:` and `spent:`, folded one row at a time | `config.store`, `finding.model`, `log.store`, `state.store` |
