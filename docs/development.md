...
Folders cost it nothing either. When `src/` regrouped by feature and the hook's three imports moved into `src/state/`: 21.3 / 20.6 / 21.0 ms flat against 21.4 / 20.6 / 20.4 ms in folders, three rounds of 30 runs per arm, paired, both trees on the same disk. A first pair with the flat copy on `tmpfs` read 2 ms in favour of the folders, which was the disk and not the layout: what the hook pays for is a module, never the depth of its path. Moving the boundary guard into `src/boundary/` and the fallback out of `worker.client.ts` into `fallback.client.ts` moved nothing either: 27.5 / 25.6 / 27.3 ms before against 27.7 / 25.7 / 27.2 ms after on the 100-line file let through, and 29.0 / 28.5 / 30.6 against 28.7 / 28.5 / 30.3 ms on the 500-line file denied, three rounds of 30 runs per arm, paired, log on, both trees on one disk; the hook imports the same four modules by another path.

What a terminal may be shown leaving `state.store.ts` for `src/cli/terminal.reporter.ts`, 44 lines neither hook imports, costs nothing the harness can see: a 100-line file let through, 24.9 / 24.8 / 24.9 ms before against 23.7 / 24.1 / 23.8 ms after; the 500-line file denied by lines, 27.7 / 27.1 / 27.0 against 27.6 / 26.6 / 26.3 ms; the handoff hook on a turn that crosses nothing, 23.0 / 23.6 / 23.2 against 23.0 / 23.2 / 23.1 ms. Three rounds of 30 runs per arm, paired, log on, a 300 KB transcript, both trees on one disk; two identical trees measured the same day sat 0.1–0.7 ms apart. Both hooks import the same four modules, and what they no longer parse is worth at most the millisecond of the allow path, at the edge of that floor.

The measure of a file leaving `config.store.ts` for `src/measure/measure.helpers.ts`, a module both hooks now load, the `Read` gate directly and the handoff hook through `handoff.store.ts`, costs **1.1–2.2 ms per `Read` that reaches Node**: a 100-line file let through, 25.0 / 24.4 / 25.0 and 24.6 / 24.9 / 24.9 ms before against 27.2 / 26.0 / 26.4 and 26.1 / 26.6 / 26.6 ms after; the 500-line file denied by lines, 27.7 / 26.8 / 28.3 and 26.8 / 27.4 / 28.3 against 28.8 / 28.1 / 29.6 and 29.0 / 28.8 / 29.4 ms; the handoff hook on a turn that crosses nothing, 23.5 / 23.5 / 23.3 against 23.7 / 23.8 / 23.2 ms. Six rounds of 30 runs per arm for the gate and three for the handoff hook, paired, log on, a 300 KB transcript, both trees on one disk, every round of the gate after above every round before; two identical trees measured the same day sat 0.1–0.7 ms apart. That is more than the 0.7–1.0 ms the same split had measured earlier that day in a throwaway tree, and above the 0.85 ms a module cost when `state.store.ts` became three files; the handoff hook, which swapped the 308 lines of configuration it never used for the 50 it does, sits inside the floor. On the path that never starts Node, the log off and no adapter, nothing moves.
