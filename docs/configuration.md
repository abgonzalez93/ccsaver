...
Two numbers decide what the hook denies: `maxLines`, and `maxTokens`, which counts bytes/4. A whole-file `Read` past either one is denied if the file is inside the plugged root (one outside it never is: no skill could take it) and `bulk-read` would take it: a secrets file by name or place, one holding a private key or an access token, or a binary the first 8 KB did not reveal is let through with the reason `untakeable` in [the event log](events.md), because `bulk-read` refuses those and a denial nothing can delegate only costs a request. A file past the byte limit is never opened to count its lines, only, when it is about to be denied, to see whether it holds a secret, and past 1 MB not even for that: it is denied unread, as every file past the byte limit was before. The defaults are 350 lines and 8,000 tokens, which is this repository's own house style rather than a measured optimum.
