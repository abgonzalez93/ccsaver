...
ccsaver sets `NODE_COMPILE_CACHE` (the `cache/` folder of the state folder) for the hook and the command, `NODE_USE_ENV_PROXY=1` for the command behind a proxy, and five for the fallback: `MAX_THINKING_TOKENS=0`, `CLAUDE_CODE_EFFORT_LEVEL=low`, `CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1`, `CLAUDE_CODE_PROMPT_CACHE_TTL=5m` and `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`.
