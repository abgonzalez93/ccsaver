...
Two numbers decide what the hook denies: `maxLines`, and `maxTokens`, which counts bytes/4. A whole-file `Read` past either one is denied if the file is inside the plugged root (one outside it never is: no skill could take it), and a file past the byte limit is never opened to count its lines. The defaults are 350 lines and 8,000 tokens, which is this repository's own house style rather than a measured optimum.
