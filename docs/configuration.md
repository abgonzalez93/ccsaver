...
- `ccsaver handoff off` silences it, `ccsaver handoff on` brings it back, `ccsaver handoff <tokens>` moves the limit, and `ccsaver handoff` alone says what is set and what is kept. On is the default and needs no file: `handoff.json` (600) is written on the first change. One that is there but malformed or unreadable stops every one of these with its name, and leaves the hook silent with a `crash` in the [event log](events.md), never read as the default. Typed with the value already in force, each one answers `already` and writes nothing.
