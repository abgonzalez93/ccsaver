...
The launcher is a script, not a symlink, because the folder Claude Code installs into carries the version in its name. On each call it reads the installed version from Claude Code's record, `installed_plugins.json` under `CLAUDE_CONFIG_DIR` or `~/.claude`, and runs that one, never the newest folder in the cache: an uninstalled plugin stays on disk for about 14 days, and the launcher then says `ccsaver is not installed` and exits 1. The record is undocumented: a shape it does not know stops it, naming the file. It keeps the folder it found in `launcher-root` inside the state folder and trusts that file while it is newer than the record, which every install, update and uninstall rewrites, so a terminal pays the `node` that reads the record once per install rather than on every call, [47 ms a command instead of 64](notes/development.md#the-terminal-launcher); a folder the sweep took is noticed all the same, because the launcher checks the file it is about to run. Inside a session it steps aside for the `bin/` the session loaded, before either, so a skill runs the version its session carries, at [2 ms](notes/development.md#the-terminal-launcher) on a short `PATH`; a session that updated the plugin keeps the version it loaded until `/reload-plugins`, which is why the remembered folder is looked at after the `PATH` and not before. `rm -f ~/.local/bin/ccsaver` removes the launcher, and the `rm -rf` of [Uninstall](../README.md#uninstall) takes `launcher-root` with the rest of the state folder.
...
- Every request caps the answer with `max_tokens`: 2,048 tokens for `bulk-read`, because an answer in bullets never needs more and a longer one would not fit the ~30 KB the Bash tool shows inline, and 8,192 for `code-write`; a provider's own default never decides, and a body past 4 MB counts as no answer. An answer cut at the `code-write` limit says so before it falls back, because half a file is no file; one cut at the `bulk-read` limit is kept and the note says the worker hit its limit, because [bullets with their citations checked](notes/measurements.md#an-answer-cut-at-the-output-limit) are worth more than a second call, and a higher limit would not have stopped a model that does not stop.
- Every request sends `max_tokens` and a `temperature` of 0.2, which every OpenAI-compatible endpoint measured so far takes; OpenAI's own o-series and gpt-5 models refuse `max_tokens` (they take `max_completion_tokens`) and a temperature other than 1, so with one of those as the worker every call answers 400 and goes to the fallback, and `doctor` says so. The request stays as it is until such an endpoint is measured here.
...
Without a worker, or whenever it fails, times out (30 s: the slowest real call measured took 19 s) or cuts a `code-write` short, the call goes to Claude Haiku through your own Claude Code binary: the one the session runs on (`CLAUDE_CODE_EXECPATH`, an undocumented variable observed in Claude Code 2.1), then `claude` on your `PATH`.
...
- The permissions of the state folder and of every file it keeps there: the key, `plugged` and `worker.json`, all 600, plus `prices.json` and `handoff.json` at 600 and your own `adapters/` and `handoff/` at 700 once each one exists — a folder made by hand is world-readable, and the adapters in it carry your house style. Stricter passes: what the check asks is that you can read them and nobody else can, so a key you set to 400 is `ok` and the line shows the mode it found. The log is the exception, because the hook appends to its files: its folder must be 700 and its files 600.
- Whether `permissions.allow` carries `Bash(ccsaver bulk-read *)` and `Bash(ccsaver code-write *)`, the two rules the [README](../README.md#what-the-two-permission-rules-grant) asks for, or a wider `Bash(ccsaver *)`, the `:*` spelling counting as the ` *` one: in `~/.claude/settings.json` (or the `settings.json` under `CLAUDE_CONFIG_DIR`), or in `.claude/settings.json` or `.claude/settings.local.json` of a plugged project, which is where "don't ask again" writes them. A rule in no file, a user file missing or one that is not JSON is a `warn` that names the file, never a `FAIL`, because the files are Claude Code's and ccsaver never edits them. Without the rules Claude asks before every delegation and refuses one in print mode.
- The [handoff](handoff.md), on or off, its limit and the point where the session hands off (`handoff:`); a `handoff.json` that is there but cannot be trusted is a `FAIL` with its name.
...
- The worker, with a probe shaped like a real call: same temperature, the `max_tokens` of a `code-write`, a system message, so a provider that would refuse the real thing fails here. 200 = the key works, 401/403 = rejected, 400 = the key or the request rejected, which is what Google answers to a wrong key, and a timeout is told from a host that cannot be reached. A rejection carries the first 200 characters of the `error.message` the endpoint sent, so a `max_tokens is not supported` from a provider whose newest models take `max_completion_tokens` only is read as that, not as a wrong key. A `worker.json` it cannot trust fails the line.
...
Every field is optional. `format` runs from the project root with the written file appended. A bare name (`biome`) is looked up on your `PATH`; a path, relative or absolute, runs only when it resolves inside that root, and one that does not (`../tools/fmt.sh`, `/opt/tools/fmt`) is not run at all: the line says so and the file is left unformatted, because the one command ccsaver runs without asking must not be reachable from outside the project. The path is judged as written, symlinks not followed: a `node_modules/.bin/biome` that pnpm links into a workspace store above the root still runs, because it is the project's own tooling, which is what plugging in trusts. It runs for at most 60 s and never past what is left of the 115 s the command gives itself inside the 120 s of a Bash call, keeping at least 1 s, and may print at most 1 MB; it runs with a bare environment, `PATH`, `HOME`, `LANG` and `TMPDIR` only; a failing formatter is reported with its exit code and the first 400 characters it wrote to stderr, which reach the session and never the log, not fatal; `after` lines are printed as `next:` commands for the model to run, with `{target}` replaced by the absolute path, shell-quoted when it needs it, so do not add quotes of your own; `maxLines` and `maxTokens` move the hook's thresholds, which [Limits](#limits) covers; `rewrite`, `true` or absent, turns the denial of a whole-file `Read` into the first `maxLines` lines of the file, with a note in the model's context that says how long the file is and what it got ([what that is worth](notes/measurements.md#rewriting-a-denied-read-into-its-first-lines)); and an adapter, like the log, sends every `Read` to Node, [24 ms instead of 4](notes/development.md#hook-overhead-per-read): the `sh` gate answers alone only with the default limits. [`adapters/strict-ts.json`](../adapters/strict-ts.json) is a working example, and what its `rules` are worth carries [its own note](notes/measurements.md#the-rules-of-an-adapter): they reach the worker and change what it writes, 3 runs of 4 against 0 of 4, while the import they were once credited with came out right with them and without them.

## Limits

Two numbers decide what the hook denies: `maxLines`, and `maxTokens`, which counts bytes/4. A `Read` with `offset`, `limit` or, of a PDF, `pages` is a ranged read and passes; a notebook is JSON to the hook, its lines counted and not its cells, so an `.ipynb` over the limits is denied like any text file; a whole-file `Read` past either one is denied when the file is inside the plugged root and `bulk-read` would take it, or, under an adapter with `rewrite`, answered with the first `maxLines` lines of the file and a note that says so. One outside the root is never denied, because no skill could take it; one `bulk-read` would refuse, a secrets file by name or place, one holding a private key or an access token, or a binary its first 8 KB did not reveal, is let through with the reason `untakeable` in [the event log](events.md), because a denial nothing can delegate only costs a request. A file past the byte limit is never opened to count its lines, only, when it is about to be denied, to see whether it holds a secret, and past 1 MB not even for that: its name and its place are still checked, its text is not, so it is denied unread, as every file past the byte limit was before, unless it is named or placed like a secret. The defaults are 350 lines and 8,000 tokens, which is this repository's own house style rather than a measured optimum: delegation [starts to pay](notes/measurements.md#where-delegation-starts-to-pay) at roughly 2,000 lines, so below that a denial is a nudge towards Grep or a ranged read, and a saving only when the model then lets the file go ([what a denial costs after the message](notes/measurements.md#what-a-denial-costs-after-the-message)).
...
The measurement is taken at `plug` and at every `doctor`, and nowhere else: the hook never walks the project, because it answers every `Read` inside 30 ms, so a repository that grows past its limits says nothing until someone runs `doctor` again; nothing is stored between runs, so the answer is always that day's.
...
The limit, the margin, the point where a session hands off, what the hook does at each event and the states of a session have [a page of their own](handoff.md). `ccsaver handoff off` turns it off, `ccsaver handoff on` brings it back, `ccsaver handoff <tokens>` moves the limit, and `ccsaver handoff` alone says what is set, where the point sits and what is kept; off costs what an unplugged project costs, because `hooks/gate` reads the switch in `sh` before Node starts.
...
| `HTTPS_PROXY`, `HTTP_PROXY` | the launcher | Node's `fetch` ignores a proxy of the environment unless `NODE_USE_ENV_PROXY=1` is set, so behind one every call to the worker would fail as unreachable and pay the fallback; when either is set (upper or lower case) and `NODE_USE_ENV_PROXY` is not, `bin/ccsaver` sets it to `1` for the command it starts and never overrides a value already there. Node 22.22 and 24.16 list it; whether the 22.18 floor does is unverified, and a Node that does not know the variable ignores it, so the limit stands there |
...
ccsaver sets `NODE_COMPILE_CACHE` (the `cache/` folder of the state folder) for the hook and the command unless one is already set, `NODE_USE_ENV_PROXY=1` for the command behind a proxy, and five for the fallback: `MAX_THINKING_TOKENS=0`, `CLAUDE_CODE_EFFORT_LEVEL=low`, `CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1`, `CLAUDE_CODE_PROMPT_CACHE_TTL=5m` and `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`.
