...
| `HTTPS_PROXY`, `HTTP_PROXY` | the launcher | Node's `fetch` ignores a proxy of the environment unless `NODE_USE_ENV_PROXY=1` is set, so behind one every call to the worker would fail as unreachable and pay the fallback; when either is set (upper or lower case) and `NODE_USE_ENV_PROXY` is not, `bin/ccsaver` sets it to `1` for the command it starts and never overrides a value already there. Node 24.16 honours it; on the 22.18 floor an unknown variable is ignored and the limit stands |
