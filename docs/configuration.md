...
- Whether `permissions.allow` of `~/.claude/settings.json`, or of the `settings.json` under `CLAUDE_CONFIG_DIR`, carries `Bash(ccsaver bulk-read *)` and `Bash(ccsaver code-write *)`, the two rules the [README](../README.md#what-the-two-permission-rules-grant) asks for, or a wider `Bash(ccsaver *)`: a missing one, a missing file or one that is not JSON is a `warn` that says so, never a `FAIL`, because the file is Claude Code's and ccsaver never edits it. Without the rules Claude asks before every delegation and refuses one in print mode.
