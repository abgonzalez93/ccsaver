...
import { attempt, DEFAULT_LIMITS, isRecord, messageOf, parsed } from "../src/state.ts"
...
export const changelogOf = (mine: string, parent: string, section: string): string => {
  ...
  const kept = parts.findLastIndex((_, index) => fits(upTo(index + 1))) + 1
  return upTo(Math.max(kept, intro === "" ? 1 : 2))
}
...
const versioned = (): string | undefined => {
  ...
  const section = sectionOf(after, date, message)
  const stale = [...MANIFESTS, CHANGELOG]
    .flatMap((path): Edit[] => {
